...
| `phase3_m3_verification.py` | P3-5+ 全周期 Level C M3 验证；经 `coupling/conjugate.py` 的 `wall_bc="thermal_grad"`（Grad 正则化热壁）跑满周期、拟合 `T_s_hat/q_g_hat` 对解析导纳，输出 `results/m3/<timestamp>/`（summary/report + 合同 §9 `timeseries.h5` 全 metadata，`p_hat` 仅 HDF5 诊断）。scoped 10kHz：相位门 PASS、幅值门边界（分辨限），见 `docs/Phase_3/M3/M3_Verification_Report.md` §9。 |
...
| `phase4_m4_endtoend.py` | **P4-5/E2：M4 端到端远场验证**（读校准声学 config；链全冻结零调参）：M3 canonical `T̂_s`（digest 26be2fde 硬编码留档）→ compact-source 映射 → `dp̂_phys=0.5997 Pa` → **G 盲用预补偿**（0.1580@+152.4°，§12.3 冻结；handoff 重现 +1.18% 为反自标定活证）→ 校准粗域（复用 `run_chain(dp_pa_override, return_band)`）→ 控制面双通道（动量 primary / 出射假设 cross-check，−0.36%）→ K0 kernel 600λ tiling → 远场 vs **R1 compact thermophone 平面波公式**。E2 幅值 +2.28%<10%、相位 1.21°、R2 双控制面 0.18%；SPL 86.6 dB ±0.46 dB[M3]；**含粗域 CV 声能通量审计**（`I(y)=½Re[p̂v̂*]`，带内闭合 ~1%，`GAS_CV_AUDIT_DIAGNOSTIC_QUANTIFIED`）。写 `results/m4/<run_id>/summary.json` + physics-core digest（`cbcf7d738ede`）。门测试 `verification/test_phase4_m4_endtoend.py`。立项 §13 / M4 报告。 |
