...
def _check_equilibrium_for_mapping(config):
    ...
    for mach, direction in [(0.0, np.array([1.0, 0.0])), (0.01, np.array([1.0, 1.0]) / math.sqrt(2.0))]:
        ...
        f_eq, g_eq = equilibrium_fg(rho, u, theta, mapping.lattice.S)
        f_mom = raw_moments(f_eq, max_order=4)
        ...
        g_mom = raw_moments(g_eq, max_order=2)
        ...
        macro = recover_macro(f_eq, g_eq, D=mapping.lattice.D, S=mapping.lattice.S)
