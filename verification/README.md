...
| `test_phase3_m3_verification_script.py` | P3-6 M3 verification 脚本机制测试（4 项）：HDF5/digest/能量审计、恢复壁温门与 CLI scoped/failed 退出语义；不断言权威 M3 数值。 |
...
| `test_phase4_d3_map_chain.py` | **P4-D3 D3-4(iii)/(iv) 链路门（2 项绿）**：映射→软源→粗域链路 smoke（`scripts/phase4_d3_map_chain_smoke.py`，ny=384/8 周期测试档）——10× 驱动 G 线性（`|G|` 偏差 <1%、相位 <0.5°，加性软源 rig 常数）、单向 `w⁻/w⁺<0.05`、行波干净（相位拟合残差 <0.02 rad、带平坦 <2%）、质量漂移 <1e-4；**(iv) 认证：`c_si_over_air` 与真空气 347.0 差 <2%**（P2-6 口径；校准介质实测 +0.17%@ny512）。范围：链路非 M4 端到端。 |
| `test_phase4_m4_endtoend.py` | **P4-5 E2 门（2 项）**：E2 幅值/相位、固定绝对观察点三点最大 R2、导数双通道幅值/相位、handoff；另用反例证明通道相位超门会失败。权威实测 1.62%/0.92°、R2 2.63%、通道 −0.36%/−0.07°。 |
| `test_phase4_kirchhoff.py` | **P4-4 K0 门（4 项绿）**：`farfield/kirchhoff_2d.py` 的 manufactured fixture 锚定（复用 `scripts/phase4_kirchhoff_verification.py` builders + `configs/phase4_kirchhoff_fixture.yaml`）——① 圆柱波（积分+H² 种类+`(−i/4)` prefactor 三合一，5 观察点 0.08%/0.03°）；② 离散收敛（欠采样 1.5/λ 必须错 >50%、加密到门内）；③ 相位约定（平面波经**速度通道**，相位 −kh，0.49%/0.50°）；④ **错核反例**（`H₀^{(1)}` 在 `e^{+iΩt}` 下=入射波，重构误差 104% 必须 >0.5——判别力）。门 `<2%/<2°`（合同 §9.3，仅约束 kernel）。 |
