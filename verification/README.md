...
| `test_phase4_m4_endtoend.py` | **P4-5 E2 门（1 项绿，~110 s）**：M4 端到端链（`scripts/phase4_m4_endtoend.py`，全冻结零调参）vs R1 参考——E2 幅值 <10%（实测 +2.28%）、相位 <10°（1.21°）、R2 控制面自洽 <5%（0.18%）、双通道差 <10%（−0.36%）、冻结 G 盲用 handoff 重现 <5%（+1.18%）。M4 措辞 `PASSED_WITH_SCOPED_RISK` 属报告、此测试只绑 E2 数值。 |
