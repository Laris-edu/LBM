...
```text
scripts/phase2_acoustic_attenuation_sweep.py
```
...
```text
python -m scripts.phase2_acoustic_attenuation_sweep
python -m scripts.phase2_acoustic_attenuation_sweep --dynamic --max-dynamic-candidates 2
...
该方向已在 `docs/Phase_2/closure/Phase2_D2Q37_Ghost_Orthogonal_Trace_Closure.md` 中推进到 symbol-level prototype。推导把 trace closure 写为：
