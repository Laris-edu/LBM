...
**口径注记(symbol vs dynamic)**:动态 Prony(生产 P2-6)给 diag≈1.31;此前 symbol 一步本征值口径(caliber/anisotropy 诊断)给 diag≈1.265。两者在 published `chi` 一致(1.41),在 `chi*` 对角相差 ~3%;**动态/生产口径为权威**,差异在已接受的 diagonal GO-RISK 边界内,symbol 口径的对角值待复核(不影响升级:x/y=1.000 两口径一致、硬门全过)。
