...
**复现脚本**：`scripts/diagnose_phase2_recursive_regularized_closure.py`、`scripts/diagnose_phase2_acoustic_attenuation_caliber.py`、`scripts/diagnose_phase2_acoustic_attenuation_anisotropy.py`
...
**脚本**:`scripts/diagnose_phase2_acoustic_attenuation_caliber.py`、`scripts/diagnose_phase2_acoustic_attenuation_anisotropy.py`
...
权威 run `20260621T142501Z`(脚本 `scripts/diagnose_phase2_acoustic_attenuation_anisotropy.py`)给出两条量化边界证据:
...
**脚本**:`scripts/diagnose_phase2_high_mode_acoustic_rr.py`
...
**脚本**:`scripts/diagnose_phase2_rr_baseline_promotion.py`
**口径**:**仅评估,未改任何默认**;翻转默认 baseline 是需单独拍板的大改(同步 core/config/unit mapping/文档)。
...
**决定(2026-06-22):已批准把 Pr=2 作有界鲁棒性 GO-RISK,并升级 RR 为默认 baseline。** 升级**执行为后续独立 PR**(本 PR 仅含残差收口 + 升级评估,默认仍 `measured`+`current_zero`)。执行清单:配置翻转为 RR(`chi*=1.1052362846829455`)+ `…_current_zero_baseline.yaml` 快照;P2-7 把 Pr>1 重分类为鲁棒性 GO-RISK(硬门只卡 Pr≤1);P2-6 诊断衰减升到本征值/Prony 口径;M2 报告更新(默认=RR、声衰减 x/y→1.0、携带已接受 GO-RISK:对角 1.265 / high-mode / Pr=2 鲁棒性);全套 M2 验证 + 回归。core 代码默认(策略未指定 fallback)保持 `measured`/`current_zero`,只由生产配置选 RR。升级 ≠ 声明 final production pass。
