...
| Production physics validation | IN PROGRESS / GO-RISK | D2Q37 输运候选、P2-5 x/y/diagonal thermal heat-flux、P2-6 x/y/diagonal sound speed/gamma 和 P2-9 真实 Galilean 已通过；high-mode acoustic `full_modal_target` diagnostic smoke 已覆盖 `Pr/tau32`、Mach/background 和相邻 wave-number branch 的 speed/gamma 外推，但 matched 声衰减 measured/reference 仍显著失配，且 full-modal closure 仍不是 local production collision。 |
...
- 已新增 `scripts/diagnose_phase2_high_mode_acoustic_rr.py`(run `20260622T051622Z`),用本征值口径评估残差 #3(high-mode 声学过阻尼 + RR 组合),见 RR 文档第 9 节:high-mode 过阻尼 5–12× 真实、闭合色散驱动(`excess≈1.415·((k/k1)²−1)`,非 filter)、RR 无关(RR 只修 mode1)、对声学紧致 10 kHz 目标物理无关;镜像决策 A 接受为有界 GO-RISK 边界。纯诊断,不改 baseline。
- 已新增 `scripts/diagnose_phase2_rr_baseline_promotion.py`(run `20260622T060549Z`),评估 RR(chi*)升级为默认 baseline(评估口径、未翻转任何默认),见 RR 文档第 10 节:RR @ chi* 过所有硬门 + ghost + 长窗口,P2-7 整体扫描 `FAILED` 但仅 Pr=2 合成极值失败(RR 5.24% vs baseline 4.61%);**空气工作点 Pr=0.706 RR 0.18% 通过**(优于 baseline 0.41%),Pr≤1 全过。修订判定:RR @ chi* 过所有物理相关硬门,唯一失败是非物理 Pr=2;是否升级取决于 P2-7 的 Pr=2 对空气目标算硬门还是鲁棒性 GO-RISK 的口径策略(待拍板)。纯诊断,不改 baseline。
...
10. 继续扩展 `docs/M2_Verification_Report.md` 的单位映射、lattice moment、equilibrium residual、声学风险判定表。
