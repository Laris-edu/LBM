...
| 2026-07-03 | 新增 `scripts/phase2_closure_export_k_window.py` 与 `docs/Phase_2/closure/Phase2_Conductive_Export_K_Window.md`（Phase_3 P3-6 幅值边界驱动的读出链机理诊断，digest `68042ca3…`）：传导热流**导出**比是 (tau,k) 窄带点标定——raw central energy-flux 矩由平衡-streaming 有限差分伪影主导（`M_eq0≈0`、`M_art/M_full≈1.32–1.41`、∝k^2.10、近乎 α 无关、标定点 ~16× 物理通量=1/factor），窗形=raw 超线性 k 形 ×（k=0.196 处）谱修正悬崖 ×0.3201 的巧合乘积；k 鲁棒矩基导出判定**无廉价工程解**（减伪影/常数重标/谱反卷积逐项否决，唯一真路=碰撞/闭合层建模 neq k² 通道）。**diagnostic、不改任何配置/读出/baseline；全部 M2/M3 结论在标定 (tau,k) 点上不变**——该分析解释其为何成立与外推边界。 |
