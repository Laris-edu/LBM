...
下一步 = **WP3 首轮 8 信息单元 Go/No-Go（合同 §14,≈20–40 run;启动属用户决策）**
——A2a 矩阵应围绕动力学非线性残差的标度/来源设计;WP4 届时走 SCOPED_GO。
