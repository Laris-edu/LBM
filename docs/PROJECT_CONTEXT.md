...
## 5. 下一步优先级（M3 收尾决策后，2026-07-03）

1. **P4-0 合同冻结**：M4 gate 定义、Kirchhoff 远场路线、控制面口径；`docs/Phase_4/` 脚手架（instruction/STATUS/README/Output Guide）；本文阶段指针与新会话最小读取切换。产出必须携带 `M3_Closure_Decision.md` §3 授权边界。
2. **Phase_4 硬前置：气体域开顶/无反射边界**：当前 y 向周期，远场外推必须先解决；同时清偿 P3-4 以来挂账的气侧 CV 能量审计。
3. **停放项（条件重启，见 `M3_Closure_Decision.md` §4）**：k 鲁棒传导导出（碰撞/闭合层建模 neq k² 通道，机理见 `docs/Phase_2/closure/Phase2_Conductive_Export_K_Window.md`）、tau 鲁棒 Grad 壁重构、Level A/B/C 清晰 `<5%` 幅值门——仅论文级/频率鲁棒需求时立项。
4. **延后项**：非紧致/高 k/`Pr>1`、RR high-mode 输运重标、RR 热 dispersion 泛化。
