...
1. 本地 recursive-regularized 闭合（应变率偏量 + `div` 迹,完整门况 run `20260621T083625Z`;测量口径 + 过约束 run `20260621T142249Z`;各向异性边界 + 紧致 run `20260621T142501Z`;残差 #3 run `20260622T051622Z`,见 `docs/Phase_2/Phase2_D2Q37_Recursive_Regularized_Closure.md`)已表征,强 GO-RISK 候选。**三条残差全部收口**:**(b) 窗口漂移**=测量侧伪影(F1,本征值/Prony 口径 x/y=1.000 精确、diagonal=1.265);**(a) diagonal**=D4 局部线性闭合不可约过约束(T1),已按决策 A 接受为有界结构性 GO-RISK 边界(45° 上界 1.265、轴向精确、紧致下对 `p_hat` 可忽略),局部线性 stencil 不再试;**(d) high-mode**=残差 #3,色散驱动、RR 无关、对紧致 10 kHz 目标物理无关,镜像决策 A 接受为有界 GO-RISK;**(c) P2-7**:整体扫描失败但**仅 Pr=2 合成极值**(RR 5.24% vs baseline 4.61%);目标气体空气 Pr≈0.706<1,**空气工作点 RR 0.18% 通过**(优于 baseline 0.41%)、Pr≤1 全过。baseline 升级评估已完成(run `20260622T060549Z`):RR @ chi* 过**所有物理相关硬门**(含空气点 P2-7)+ ghost + 长窗口,唯一失败是**非物理 Pr=2**。是否升级不再卡物理门,取决于**验证口径策略**:Pr=2 对空气目标算硬门 vs 有界鲁棒性 GO-RISK(物理支持后者)。**决定(2026-06-22):已批准把 Pr=2 作鲁棒性 GO-RISK 并升级 RR 为默认 baseline(选项①)。** 升级**执行为后续独立 PR**(本 PR 仅含残差收口 + 评估,默认仍 `measured`+`current_zero`);执行清单见 RR 文档第 10 节(配置翻转 chi*+`…_current_zero_baseline.yaml` 快照 + Pr>1 重分类为鲁棒性 GO-RISK + P2-6 升本征值/Prony 口径 + M2 报告 + 全套验证;core 代码默认保持 fallback,只由生产配置选 RR)。升级 ≠ final production pass;默认将携带已接受的声衰减 GO-RISK(对角 1.265、high-mode、Pr=2 鲁棒性)。后续另含:完善 HDF5 probe 供 Phase_3。
