...
- **D3-4 端到端：E2 PASSED → M4 `PASSED_WITH_SCOPED_RISK`** ✓（§13；2026-07-11 审查修订）：固定同一绝对观察点并取三点最大 R2 后，E2 幅值 +1.62%<10%、相位 0.92°、R2 2.63%<5%、通道差 −0.36%/−0.07°；**SPL 86.67 dB ±0.46 dB[M3]**；digest `d69bf24d881e`。**D3 主线闭合**（D3-0→D3-4 全过；M4 报告 `M4_Verification_Report.md`）。
...
**权威 run** `results/m4/20260711T063735Z`（digest `d69bf24d881e…`）；交付 `scripts/phase4_m4_endtoend.py` + `verification/test_phase4_m4_endtoend.py` + **`docs/Phase_4/M4/M4_Verification_Report.md`**（合同 §11.2 七节）+ `M4_Run_Summaries.md`。2026-07-11 修订把 R2 改为两控制面共享同一绝对观察点、三点最大值，并把通道相位 `<10°` 纳入 gate。
...
| 门 | 实测 | 合同门 | 余量 |
|---|---|---|---|
| E2 幅值误差 max（3 观察点 20–100λ） | **+1.62%** | `<10%`（硬门 4） | 6.2× |
| 远场相位误差 max | 0.92° | `<10°`（建议门） | 10.9× |
| R2 控制面位置自洽（同一绝对观察点、三点最大） | 2.63% | `<5%`（建议门） | 1.9× |
| 双通道幅差 | −0.36% | `<10%`（合同 §6） | 28× |

**绝对声明（携带 M3 带）**：法向远场 `p̂≈0.60 Pa`、**SPL 86.67 dB ±0.46 dB[M3 ±5.4%] ± ~0.4 dB[源实现 ±5%]**，10 kHz、每侧、x 周期法向出射。预算仍取保守 ±7%；R2 2.63% 为修正后的主要控制面敏感性量，但仍低于 5% 门。
