...
boundary/open_sponge.py                                 # D3-2 扰动衰减吸收层（生产 sponge）✓
boundary/（细/粗界面耦合构建器）                          # D3-3 待建
```
...
- **下一步 D3-3（最大风险）**：细热区↔粗声域界面耦合（f rescale + 宏观匹配 + 通量守恒）；最小界面 fixture 先判死活。
...
**待决策**：(1) ~~做声学域简化碰撞 core 步~~ **已执行，见 §8**；(2) 收尾 (a)——D1死 / b′死 / D3"稳定解决、认证未闭合"合成完整的"全局谱修正 LBM 做远场的系统性障碍 + 一条有实质进展但未闭合的多域出路"方法学地图。
