...
| A2a-STRICT_B 原协议复测 | 方案=`docs/Phase_5/a2a_strict_b_experiment_plan_v1.0.md`；报告=`docs/Phase_5/a2a_strict_b_report.md`；runner=`scripts/phase5_a2a_strict_b.py`；QS 参照=`reference/strict_face_robin_qs.py`；refpack 生成器=`scripts/phase5_a2a_strictb_refpack.py`；配置=`configs/phase5/a2a_strict_b/`；测试=`test_a2a_strict_b_contract.py`（12 项） | **已执行（2026-08-19 用户当日授权；判决权保留用户）**：wet reference pack=`archive/a2a_strict_b/`（五重放位级一致+SHA-256）；判决 run B 机 `20260819T155402Z`（105 min，合法性全绿，冷锚 3.87%/3.87° PASS）；四点 d_OP 仍负（−1.05/−2.52/−3.66/−4.74%）、一致上移（+0.13→+0.57 pp）、C_R≈0.48、机械候选=`NOT_RESOLVED_CANDIDATE`（g0_scope=G0_FENCE_PENDING_USER）；精选摘要归档 `archive/M5_runs/a2asb_20260819_B/`、signals 镜像 `results/mirror_from_B/a2a_strict_b/`；正式判决只读 STATUS |
| A2a-STRICT_B 系综轴扫描 | 计划=`docs/Phase_5/a2asb_ensemble_scan_plan_v1.0.md`；报告=`docs/Phase_5/a2asb_ensemble_scan_report.md`；runner=`scripts/phase5_a2asb_ensemble_scan.py`；配置=`configs/phase5/a2a_strict_b/ensemble_scan_10k_dx2p6.yaml`；测试=`test_a2asb_ensemble_scan.py`（8 项） | **已执行（2026-08-20 用户指令当日闭合）**：B 机 `20260819T193831Z`（94.9 min，wet 点/冷锚 checkpoint 位级复用）；`ENSEMBLE_AXIS_PARTIAL`——质量轴主导（斜率 0.956/0.959 pp/%、盈余点符号翻正、切线锚和解 0.006-0.007 pp），静态平坦门败（连续 QS-1 自带 ~55.5% 质量斜率→异常=密度超额响应 1.80×+随 Θ 偏置）；归档 `archive/M5_runs/a2asb_ensscan_20260820_B/`、镜像 `results/mirror_from_B/a2asb_ensemble_scan/` |
| 偏置判别单元（offset lenses） | 计划=`docs/Phase_5/a2asb_offset_lenses_plan_v1.0.md`；报告=`docs/Phase_5/a2asb_offset_lenses_report.md`；runner=`scripts/phase5_a2asb_offset_lenses.py`；QS 透镜模式=`reference/strict_face_robin_qs.py`；NSF 格子通道=`reference/nsf_hot_base_linear_1d.py`（默认关闭）；测试=`test_a2asb_offset_lenses.py`（6 项） | **已执行（2026-08-20，A 机 `20260820T073200Z`，零 LBM 算力）**：`OFFSET_BEYOND_CONTINUUM`——等质量 Θ 偏置为超连续动理学残差（格子透镜下质量平坦；换本构无机制消除）；三层最终分账=报告 §3；归档 `archive/M5_runs/a2asb_offlens_20260820_A/` |
