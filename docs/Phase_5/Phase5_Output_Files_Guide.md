...
- 历史诊断例外必须在报告中显式标注，不能冒充合同 §16.1 的七文件 Gate run。`faceflux_20260817_A/` 当前只含 summary/log 精选件，正式口径为 `D1_B_BUFFER_DIAGNOSTIC`；strict-B 尚无运行目录或归档。
...
| D1 strict-B 实验执行清单 | 清单=`docs/Phase_5/strict_faceflux_candidate_b_design_v1.0.md`；实施资产、验证门和停止条件均在该清单 | **冻结设计已无损压缩为 88 行短版，用户已确认文档形式**：`STRICT_B_DESIGN_CONDITIONAL / IMPLEMENTATION_NOT_STARTED / D1_SCIENTIFIC_GATE_OPEN`。当前没有 strict-B 代码、配置、测试、run 或归档；不得预建结果目录冒充实施开始 |
