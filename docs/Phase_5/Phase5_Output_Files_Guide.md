...
| 跨栈普遍性单元 1a（碰撞算子结构轴） | 计划=`docs/Phase_5/crossstack_1a_plan_v1.0.md`；算子=`core/collision_bgk.py`（新增，生产 `collision_smrt.py` 一字不改）；链路接入+结构探针=`core/tangent_bgk.py`；runner=`scripts/phase5_crossstack_collision_scan.py`；测试 `test_phase5_crossstack_collision.py`（20 项）；报告=`crossstack_collision_report.md` | **执行中（2026-08-14 用户下达计划书当日启动；D0-7 诊断）**：A 机 preflight `20260814T090438Z_preflight` `COMPLETED`；B 机 full `20260814T090824Z_full`（22 workers，schtasks 一次性派发，两机同 commit `111f757`，D5-3）。**BGK 轴已闭合=生产工作点上无条件线性失稳**（计划书 §3 回退路径 1）；配置轴 auth 判决网格运行中。预注册 commit `111f757` 先于任何变体热态数值；逐例检查点 `results/phase5/crossstack_collision/checkpoints_*`；不改变 Gate/生产壁/写作轨 |
