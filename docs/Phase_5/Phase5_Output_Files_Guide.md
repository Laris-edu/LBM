...
| WP4 认证子矩阵 runner 族(D5-6) | A2a 点=`scripts/phase5_a2a_operating_point.py`(配置 `a2a_wp4_dc002`/`a2a_wp4_dc0075`)+ A1 全阶梯=`scripts/phase5_a1_signed_zero_mean.py`(配置 `a1_wp4_full_ladder`)+ **A5 χ 地图=`scripts/phase5_a5_chi_map.py`(净新增,配置 `a5_chi_map/a5_wp4_10k_dx2p6.yaml`)** | **已交付(2026-08-03)**:runner 参数化=配置驱动(theta_dc/label_tag/unit_label;默认逐位保持 WP3 行为);A5=G4a 耦合仪器逐字复用+冷参照 χ 轴+闭式 P₁ 反解;合同测试 `test_phase5_wp4_units.py` 7 绿;权威 run 双机派发见 STATUS §6 |
