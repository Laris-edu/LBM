...
**最后更新**：2026-08-19（用户冻结 `A2a-STRICT_B` 原协议复测短版方案：保持 A2a 工况/读出，只替换 strict-B 面边界，重跑非均匀 DC 基态增量响应并对照 QS 静态族；状态=`PLAN_FROZEN / IMPLEMENTATION_NOT_STARTED / RUN_NOT_AUTHORIZED`。既有 strict-B 科学资格未授予、分类不激活、`D1_SCIENTIFIC_GATE_OPEN`、Gate、生产壁与 `FINAL_PRODUCTION_NOT_CLAIMED` 均不变。方案唯一家=`a2a_strict_b_experiment_plan_v1.0.md`）
...
| 日期 | 单元 | 目的 | 结果 | 详情 |
|---|---|---|---|---|
| 2026-08-19 | **A2a-STRICT_B 原协议复测方案冻结（D5-9）** | 在非均匀 DC 基态上重跑 A2a 时域增量响应，以单一边界替换检验 strict-B 是否消除负趋势 | `PLAN_v1.0` 已冻结；工况=`Θ_DC {0,0.02,0.05,0.075,0.10} × ε_AC {0.005,0.02}`，重新计算 QS-0/QS-1/QS-1k 与动态残差。**仅方案交付：未授权实现或运行，不改变既有 strict-B 科学判决、Gate 或生产状态** | `a2a_strict_b_experiment_plan_v1.0.md` |
...
**D5-9 已冻结论文导向的 A2a-STRICT_B 原协议复测方案。** 当前仅完成方案，`IMPLEMENTATION_NOT_STARTED / RUN_NOT_AUTHORIZED`；若后续获授权，先完成该单一边界替换实验，再决定 Section 5 的证据强度。既有 strict-B 四项 SCOPED 裁决仍待用户，未被本方案静默清偿；生产证据继续冻结。

| # | 任务 | 说明 | 依赖 |
|---|---|---|---|
| 1 | **决定是否授权实施并运行 A2a-STRICT_B** | 按 D5-9 方案重跑原 A2a 主臂；不得从“方案冻结”推导运行授权 | `a2a_strict_b_experiment_plan_v1.0.md` |
