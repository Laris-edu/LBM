...
| `a2asb_ensemble_scan_plan_v1.0.md` | topical-plan | **系综轴扫描计划（2026-08-20，`PLAN_v1.0`）**：边界固定 strict-B 只扫基态列质量，判别原框架负趋势的主承载；网格公式、三门判读线、分类与报告行全部预注册。 | 判读线或网格变化时更新。 |
| `a2asb_ensemble_scan_report.md` | gate-report 族（诊断单元） | **系综轴扫描结果（2026-08-20，`REPORT_v1.0`；B 机 `20260819T193831Z`，94.9 min，wet 点/冷锚 checkpoint 复用）**：`ENSEMBLE_AXIS_PARTIAL`——质量轴主导成立（斜率 0.9556/0.9593 pp/%、线性残差 0.03%span、盈余点符号翻正、等质量点与切线锚 0.006-0.007 pp 和解=两轴分解升级同框架事实）；静态平坦门败（同系综连续 QS-1 自带 ~55.5% 质量斜率）→ 异常改述="密度超额响应 1.80× 连续 + 随 Θ 增长的质量无关偏置（−1.65/−3.12 pp）+ 次要边界项 ~11%"。 | 用户裁决或后续机理单元变化时更新。 |
| `literature_check_constitutive_v1.md` | 文献核查（第三轮） | **本构框架三断言核查（2026-08-20，CHECK_v1，后台代理多源检索）**：变弛豫=成熟做法 FOUND（τ(ρ) 处方 2002 年已有；**D2Q37 直系 Scagliarini 2010 源头已写出 k∝ρ**）；符号级/传递函数级误差刻画 NOT_FOUND_IN_SEARCHED_SOURCES；热声应用 18+2 篇全枚举、输运归因 NOT_FOUND、thermophone×LBM 空白仍成立。定位=情形 2+1；"首次"带检索范围限定语；Scopus/SD 全文库=第二轮待办。 | 第二轮全文库核查或定位落笔时更新。 |
| `a2asb_offset_lenses_plan_v1.0.md` | topical-plan | **偏置判别单元计划（2026-08-20，`PLAN_v1.0`）**：QS 四透镜 + NSF 格子本构通道，判定等质量 Θ 偏置的归属层；判读线/输入 digest/审计门全部预注册。 | 判读线变化时更新。 |
| `a2asb_offset_lenses_report.md` | gate-report 族（诊断单元） | **偏置判别结果（2026-08-20，`REPORT_v1.0`；A 机 `20260820T073200Z`，零 LBM 算力）**：`OFFSET_BEYOND_CONTINUUM`——四静态透镜与完整 NSF 格子本构动力学均不复现等质量偏置（NSF 格子分支 +2.38/+4.75% 反而更正）；格子透镜下残差质量平坦=超连续动理学项 −2.6~−3.0/−5.0~−5.9 pp。**三层最终分账（§3）：边界 ~11% + 本构质量轴（闭合）+ 超连续残差（最大单项，方向线索=近壁动理学层）**；换 τ(ρ) 本构无机制消除第三层。 | 后续动理学层单元或用户裁决变化时更新。 |
