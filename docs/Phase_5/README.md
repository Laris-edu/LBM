...
| `faceflux_wall_report.md` | gate-report 族（诊断单元） | **D1 候选 B 面通量壁权威结果（2026-08-17，REPORT_v1.0 闭合；A 机 `20260817T090753Z_full` `COMPLETED`）**：修复入口实现级检验。通道移除双重确认+冷态预算带命中（−2.446%），但 **`WALLFIX_NULL`**（d_OP 仅动 NSF 缺口 0.5–0.7%）——**边界能量交换语义整族排除为载体**，「离散边界项主导」经预注册复测不升级；机制窗口收窄至{形状重置/u=0}∪{体相}。faceflux=纯气侧响应仪器（G_f 比值相消）。 | 形状重置消融或体相轴立项时更新。 |
