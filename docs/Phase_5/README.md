...
| `Phase5_STATUS.md` | status | **阶段状态（2026-08-17）**：状态标签 + Gate 现值追踪 + 执行流水（做了什么/为什么/结果）+ WP4 生产数据（论文数字唯一家）+ 决策记录 + 生效风险 + 下一步。**技术细节一律在对应报告**；历史留档见 `x/phase5_execution_history.md`。 | 阶段进度、Gate 状态、决策或交付物变化时更新；新增执行单元时在“执行流水”加一行（目的/结果/指针），不写技术细节。 |
...
| `wallfix_a2a5_counterproof_report.md` | gate-report 族（诊断单元） | **A2-5 修复性反证权威结果（2026-08-11，run `20260811T085347Z_auth` `COMPLETED`）**：结构论证（四不变量锁死壁切线标量通道）+合法族实测穷举——**`WALLFIX_FAMILY_NULL`：合法壁修改全体 \|S\|≤1.1e-6 pp vs 非法 A2-5 消融 +8.82 pp（7 个量级）；A2-5 异常=湿节点整格胞重钉扎范式结构性质**。PROD 身份门 1e-5 pp 级复现 TAN。严格 B 已完成设计、未实施。 | strict-B 实施或范式结论变化时更新。 |
...
| `strict_faceflux_candidate_b_design_v1.0.md` | topical-plan | **D1 strict-B 实验执行清单（2026-08-17，88 行）**：由冻结设计无损压缩，只保留镜像半域、incoming-`g` 注热、`CONST_G→G0 admission`、比较系综、最小验证矩阵、执行顺序和停止条件。状态=`STRICT_B_DESIGN_CONDITIONAL / IMPLEMENTATION_NOT_STARTED / D1_SCIENTIFIC_GATE_OPEN`。 | 实施获批、合同或状态变化时更新。 |
