...
| `a2a_strict_b_experiment_plan_v1.0.md` | topical-plan | **A2a-STRICT_B 原协议复测短清单（2026-08-19，`PLAN_v1.0`）**：保持 A2a 工况/读出，只替换为 strict-B 面边界；重跑非均匀 DC 基态增量响应并对照重新计算的 QS-0/QS-1/QS-1k。当前仅冻结方案，未授权实现或运行。 | 单一改动、工况、判决线或实施授权变化时更新。 |
