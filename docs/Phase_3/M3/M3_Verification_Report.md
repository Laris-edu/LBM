...
### 10.5 导出窗机理闭环（2026-07-03，路线 (b) 立项决策证据）

§10.4 第 1 条的导出窗获得完整机理解释（专项笔记 `docs/Phase_2/closure/Phase2_Conductive_Export_K_Window.md`；复现 `python -m scripts.phase2_closure_export_k_window`，digest `68042ca3…`）：

- **窗形构成**：k≤0.131 段谱修正恒为 1，窗形就是 raw 矩自身的超线性 k 形；k=0.196 的表观回归 ~0.98 是 raw 增长（×2.97）与 `heat_flux_lu` 内谱修正悬崖（×0.3201，轴向 k∈[0.1389,0.1963]）的**巧合乘积，不是准确性**。
- **矩本体**：raw central energy-flux 矩由**平衡分布的 streaming 有限差分伪影**主导——`M_eq0≈0`（机器零）、`M_art/M_full≈1.32–1.41`（伪影主导且过冲、neq 反号部分抵消）、∝k^2.10、近乎 α 无关、标定点 ~16× 物理通量（=1/factor，`F/(kθ)=3.5α_lu` 记账闭合 0.6%）。真实 CE 傅里叶分量（∝k¹）只是矩的极小成分；`conductive_heat_flux_moment_factor` 是对错 k 形对象的 (tau,k) 单点重标。
- **对 M3 的意义**：多频漂移（−5.3%/+2.2%/+9.0%）的符号与排序由「衰逝层 k 谱读经该窗」机理连贯解释（幅度需衰逝谱卷积、未定量化）；`q_g_hat` 能量锚不经逐模窗、不受影响；「特征钉标定 k」的 scoped 设计获得机理正当性。
- **路线 (b) 判定：k 鲁棒导出无廉价工程解**——常数重标/换标定点（窗 ∝k）、减伪影（余项仍 ∝k² 反号）、谱反卷积（壁面衰逝谱不适定）、体相 Fourier-law 读出（验证同义反复、壁面即已否的 θ 梯度路）逐项否决；唯一真路 = 碰撞/闭合层建模 neq k² 通道并构造正确投影（成本≈Phase_2 一次闭合推导）。M3 结论与全部数值**不变**。

## 11. M3 收尾决策（2026-07-03）

用户选定**方案 (a)**：接受 scoped 边界终态（**不追认 clear PASS**）、Phase_3 转维护态、授权启动 Phase_4。授权边界、停放项与重启条件的权威出处是 `docs/Phase_3/M3/M3_Closure_Decision.md`（镜像 2026-06-22 `BOUNDED_PRODUCTION_GO` 先例）；本报告保持技术终态记录职责，不重复决策内容。
