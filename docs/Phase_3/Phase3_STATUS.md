...
**状态口径**：**P3-6「M3 收尾」完成（2026-07-02）**——④ digest `26be2fde…` run_id 无关复现（HDF5 改造后不变）；③ 合同 §9 HDF5 全 metadata + `phase3_m3_summarize.py` + Level A 动态导纳固化为提交脚本（canonical `Y −5.32%/+2.20°`，digest `02cea11e…` 两跑复现）；② **首个 Level B 动态频响**（矩通量伺服）`Z +5.47%/−2.24°` = `PHASE_PASS_AMPLITUDE_BOUNDARY`（relax 0.02/0.01 收敛一致；FD 梯度控制器被否决——矩通量超发 ~2.5×）；① finer-dx 三重否证（传导导出是 (tau,k) 窄带点标定 + Grad 壁重构在 dx1p3 tau 失稳）。**M3 终态（Phase_3 范围）：相位门三级 PASS、幅值边界 ~±5.3–5.5%（近壁读出/重构 (tau,k) 点标定极限，非调参可过）**；三级自洽（A `Y −5.32%`、B `Z +5.47%`、C `T_s_hat +5.38%`/`Y_eff −5.21%`）。Phase_3 测试 39 绿。详见 `docs/Phase_3/M3/M3_Verification_Report.md` §9/§10。**M3 收尾决策（2026-07-03，方案 (a) APPROVED，`M3/M3_Closure_Decision.md`）：接受 scoped 边界终态（不追认 clear PASS），Phase_3 转维护态，授权启动 Phase_4（待 P4-0）。**

**维护修订（2026-07-11，不改 M3 终态）**：Level C 现在强制薄膜 `dt_si` 等于单步 LBM `dt_s`，重新初始化会清空 solver 时间/ghost reference；`T_wall_K/theta_wall_lu` 改记恢复出的实际壁温，M3 加 `wall_temperature_error_K<0.01 K` 门，验证 CLI 在 `NOT_PASSED` 时非零退出。32 项相关快速回归通过；canonical 全周期复跑在 904 s 工具上限内未完成、无新摘要，故历史 `26be2fde…` 仍只作为既有 T_s handoff 证据，不宣称在新摘要 schema 下复现。
...
| 日期 | 命令或 run | 结果 | 证据 |
|---|---|---|---|
| 2026-07-11 | Phase_3 维护修订定向回归；canonical M3 尝试复跑 | `32 passed`；全周期 run `TIMEOUT/NO_ARTIFACT` | 实际壁温/0.01 K 门、时钟一致、fresh init、CLI 退出语义均有回归；904 s 超时后终止残留进程，未晋升新 digest |
...
**M3 收尾决策已做出（2026-07-03，方案 (a) APPROVED，见 `M3/M3_Closure_Decision.md`）**：接受 M3 scoped 边界终态（不追认 clear PASS），**Phase_3 转维护态**（39 测试 + digest 锚为维护基线，只修回归、不新增功能），**授权启动 Phase_4**。下一步 = **P4-0 合同冻结**（M4 gate、Kirchhoff 远场路线、控制面口径、`docs/Phase_4/` 脚手架、PROJECT_CONTEXT 阶段指针切换）；Phase_4 硬前置 = 气体域开顶/无反射边界（兼清偿气侧 CV 能量审计挂账）。Phase_4 产出必须携带决策 §3 授权边界（单频 10 kHz、dx2p6 配置、幅值 ±5.4% 误差带）。

（保留记录）P3-6 四项：④ digest 复现、③ HDF5/summarize/Level A 固化、② Level B 动态频响（`PHASE_PASS_AMPLITUDE_BOUNDARY`）、① finer-dx 三重否证。合同 §15 七项全覆盖；**M3 终态（Phase_3 范围）：相位门三级 PASS、幅值边界 ~±5.3–5.5%（近壁读出/重构 (tau,k) 点标定极限）**。
...
**候选方向（已决策，2026-07-03）**：用户选定 **(a) 接受 scoped 边界结论、进入 Phase_4**（`M3/M3_Closure_Decision.md`）；(b) 两条研究项按决策 §4 停放（重启条件=论文级、频率鲁棒 `<5%` 幅值或换 dx/tau 生产配置需求）。
...
| 日期 | 更新 |
|---|---|
| 2026-07-11 | **审查维护修复**：Level C 禁止伪 `dt_si` 时间轴；输出实际恢复壁温并增加 0.01 K M3 门；solver 重新初始化清空时间和 modal reference；M3/Level A/B/C CLI 失败非零退出。未改变 dx/tau/Grad 重构/历史 M3 终态；完整 canonical 复跑超时且无产物，相关 32 项回归通过。 |
...
| 2026-07-03 | **M3 收尾决策落档（方案 (a) APPROVED）**：新增 `M3/M3_Closure_Decision.md`（镜像 `BOUNDED_PRODUCTION_GO` 先例）——接受 M3 scoped 边界终态（相位三级 PASS、幅值边界 ±5.3–5.5%、**不追认 clear PASS**）；**Phase_3 转维护态**（39 测试 + digest 锚为维护基线）；**授权启动 Phase_4**（P4-0 待执行；硬前置=开顶/无反射边界；产出携带决策 §3 授权边界：单频 10 kHz、dx2p6 配置、幅值 ±5.4% 误差带）。研究项（k 鲁棒导出/tau 鲁棒壁重构/清晰 `<5%` 门）按决策 §4 停放。同步 PROJECT_CONTEXT §2/§3/§4/§5/§6、本文件头部/§5、M3 报告、Phase_3 README、Output Guide。 |
