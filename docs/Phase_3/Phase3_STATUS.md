...
**状态口径**：P3-5+ 热壁 BC 修复完成——根因 equilibrium-clamp 热壁 BC 已由 **Grad 正则化壁面**（`boundary/wall_thermal_grad.py`，已接入 `coupling/conjugate.py` 的 `wall_bc="thermal_grad"`）修复。M3 由『未达成（`T_s_hat +61%`）』推进到『**相位达标、幅值边界**』：Level A 导纳 `−5.3%/+2.2°`、Level C `T_s_hat +5.4%/−1.9°`（相位两级 PASS、幅值恰在 `<5%` 门上，残差是近壁分辨极限、非调参可过）。契约 plumbing 已验证、27 测试绿。详见 `docs/Phase_3/M3/M3_Verification_Report.md` §9。
...
| Phase_3 framework | `IN_PROGRESS` | `boundary/`、Level A/B config/script/test、Film ODE、Level C coupling 与 P3-5+ Grad 热壁（接入 conjugate.py）已建立；M3 报告已生成。 |
...
| Level C scoped coupling | `SHORT_SMOKE_PASSED；full-period 相位达标/幅值边界` | short smoke 通过；全周期（**Grad 壁面**，已接入 conjugate.py）`T_s_hat` +61%→**`+5.4%/−1.9°`**（相位过门、幅值门上）、`q_g_hat −0.05%`，relax 0.02/0.01/0.005 收敛。P3-4 旧 clamp 全周期为 +61%（已被 Grad 取代）。 |
| M3 gate | `相位达标；幅值边界（分辨限）` | Grad 壁面修复后：相位门**两级 PASS**；`T_s_hat` +61%→+5.4%（恰在 `<5%` 门上）、Level A 导纳 −5.3%/+2.2°。残差=近壁热梯度分辨极限（多频诊断，非调参可过）；频率鲁棒 `<5%` 需更细近壁分辨。契约 plumbing 全验证。 |
...
1. **P3-5 已完成**：`docs/Phase_3/M3/M3_Verification_Report.md` 生成，**M3 `NOT PASSED`**（契约 plumbing 已验证；Level C 动态热导纳缺口定位为 equilibrium-clamp 热壁 BC，分辨率已排除）。
2. **M3 收口（若继续追 M3 pass）**：实现 anti-bounce-back 热 Dirichlet BC 替代 equilibrium-clamp（Level A/B/C 共用该 BC）；用隔离导纳测试回归到 `<5%/<5°`，再跑全周期 Level C 复核 `T_s_hat/q_g_hat`；固化探测为 `scripts/phase3_m3_verification.py`/`phase3_m3_summarize.py` + HDF5 全 metadata。**finer dx / dx2p6 重标定已被分辨率扫描排除**。
3. **可选收紧**：Level B 动态频响长时脚本（当前未运行）；Level A 动态导纳固化为提交测试。
4. M3 收口前保持 `NOT PASSED`；不得把静态/构造 smoke 或 `q_g` 能量守恒自洽等同于 M3 动态频响 pass；`p_hat` 远场留 Phase_4。
