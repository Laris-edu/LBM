...
| `a2a_strict_b/ensemble_scan_10k_dx2p6.yaml` | **系综轴扫描配置（2026-08-20 已跑；D0-7 诊断）**：唯一变量=settle 列质量目标（五点/三点网格由 pack r(Θ) 公式生成）；判决 run checkpoint 目录复用（wet 点/冷锚零新算力）；auth 窗口/N_ref 对冻结常量硬断言；含 `ensemble_scan_smoke`。判决 run B 机 `20260819T193831Z`（`ENSEMBLE_AXIS_PARTIAL`），报告=`a2asb_ensemble_scan_report.md`。 |
