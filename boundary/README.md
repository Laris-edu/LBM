...
| `wall_face_flux.py` | code | **D1 候选 B 面温—面通量带（2026-08-17，D0-7 诊断；默认不接入任何冻结路径）**：物理 Dirichlet 数据移到 ±Δy/2 的零体积面上，物理热量=面通量公式 `q_s=G_f(θ_w−θ_1s)`（G_f 冻结冷态标称 `k_nom/d_f`，D1.42）。实现取**通量喂给缓冲行**：复用 v1.1 认证重构全骨架（per-column 质量中性/u=0/方向性 neq/平衡增量清理），唯一语义改动=行能量目标从储热库值 `c_vρθ_w` 换成 `E_streamed+ΔQ_formula`——壁温驱动增益由储能系数 `c_vρ̄_0` 变为输运系数 `G_f`，**A2-5 通道按构造缺席**（合同测试 `test_drive_gain_is_transport_not_storage` 直接断言行密度无关性）。审计包装器读数=公式通量（D1.70 按构造）。测量=`core/tangent_faceflux.py`+`scripts/phase5_faceflux_wall_scan.py`。 | 语义/G_f 口径变化时更新（任何生产采用触发 G1-W 级重认证）。 |
