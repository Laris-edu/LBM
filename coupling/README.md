...
| `conjugate.py` | code | Level C predictor-corrector coupling：从**近壁气体行**(row 1，handoff 口径)提取单侧 `q_g''`、推进 Film ODE、施加 Dirichlet 壁温、一次 Picard。`wall_bc` 选 `equilibrium_clamp`（P3-4 默认，静态宏观 smoke）或 `thermal_grad`（P3-5+ Grad 正则化壁面，解析动态近壁导纳）；`q_feedback_relax`（thermal_grad 用 ~0.02 压 Nyquist 耦合失稳，记录 `q_g=q_fb` 使能量审计自洽）。 | 耦合策略、Picard、热流提取行、`wall_bc`/`q_feedback_relax` 或壁温应用策略变化时更新。 |
